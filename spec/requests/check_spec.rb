  ...
  describe "POST /api/ver1/cards/check" do
    ...
    context '無効なパラメータの場合' do
      ...
      it '期待するエラーメッセージが格納されていること' do
        ...
        expect(json_body["error"][0]["error"]).to eq "5つのカード指定文字を半角スペース区切りで入力してください.（例：S1 H3 D9 C13 S11）"
        expect(json_body["error"][1]["card_set"]).to eq ''
        expect(json_body["error"][1]["error"]).to eq "5つのカード指定文字を半角スペース区切りで入力してください.（例：S1 H3 D9 C13 S11）"
      end
